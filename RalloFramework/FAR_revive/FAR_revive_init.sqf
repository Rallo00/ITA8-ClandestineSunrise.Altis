...
/*
	...
	3 = No revive (handled by framework)
*/
FAR_ReviveMode = FWK_ReviveType;
call compile preprocessFile "RalloFramework\FAR_revive\FAR_revive_funcs.sqf";
...
[] spawn
{
	...
	if (FAR_MuteRadio) then { [] spawn FAR_Mute_Radio; } else { };
	// Event Handlers
...
[] spawn
{
	while {true} do
	{
		if (animationState player == "helper_switchtocarryrfl" || animationState player == "AcinPknlMstpSrasWrflDnon") then
		{
			if (FAR_isDragging) then { player switchMove "AcinPknlMstpSrasWrflDnon"; }
			else { player switchMove "amovpknlmstpsraswrfldnon"; };
		};
		sleep 3;
	}
};
FAR_Mute_Radio =
{
	if (isClass (configFile >> "CfgPatches" >> "task_force_radio")) then { [] spawn FAR_Mute_TFR; } else { [] spawn FAR_Mute_ACRE; };
};
FAR_Mute_TFR =
{
	waitUntil { time > 0 };
		...
		if (alive player) then
		{
			...
			if ((player getVariable["ace_sys_wounds_uncon", false])) then
			{
				...
				waitUntil
				{
					...
					if (!(player getVariable["tf_unable_to_use_radio", false])) then { player setVariable ["tf_unable_to_use_radio", true, true]; };
					!(player getVariable["ace_sys_wounds_uncon", false]);
				};
				if ((player getVariable["tf_unable_to_use_radio", false])) then { player setVariable ["tf_unable_to_use_radio", false, true]; };
				player setVariable ["tf_globalVolume", _saveVolume];
			};
		}
		else { waitUntil { alive player }; };
		sleep 0.25;
		false
	};
};
