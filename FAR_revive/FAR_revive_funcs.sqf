...
FAR_Player_Actions =
{
	if (alive player && player isKindOf "Man") then 
	{
		...
		player addAction ["<t color=""#C90000"">" + "+Cura" + "</t>", "FAR_revive\FAR_handleAction.sqf", ["action_revive"], 10, true, true, "", "call FAR_Check_Revive"];
		player addAction ["<t color=""#C90000"">" + "+Stabilizza" + "</t>", "FAR_revive\FAR_handleAction.sqf", ["action_stabilize"], 10, true, true, "", "call FAR_Check_Stabilize"];
		player addAction ["<t color=""#C90000"">" + "+Suicidati" + "</t>", "FAR_revive\FAR_handleAction.sqf", ["action_suicide"], 9, false, true, "", "call FAR_Check_Suicide"];
		player addAction ["<t color=""#C90000"">" + "+Trascina" + "</t>", "FAR_revive\FAR_handleAction.sqf", ["action_drag"], 9, false, true, "", "call FAR_Check_Dragging"];
	};
};
...
FAR_Player_Unconscious =
{
	...
	if (isPlayer _unit) then 
	{
		...
		{
			hintSilent format["Morte per dissanguamento tra %1 secondi\n\n%2", round (_bleedOut - time), call FAR_CheckFriendlies];
		...
		if (_unit getVariable "FAR_isStabilized" == 1) then {
			...
			{
				hintSilent format["Sei stato stabilizzato\n\n%1", call FAR_CheckFriendlies];
...
FAR_Drag =
{
	...
	_id = player addAction ["<t color=""#C90000"">" + "+Lascia" + "</t>", "FAR_revive\FAR_handleAction.sqf", ["action_release"], 10, true, true, "", "true"];
...
FAR_CheckFriendlies =
{
	...
	if (count _medics > 0) then
	{
		...
		if (!isNull _unit) then
		{
			...
			_hintMsg = format["Medico vicino:\n%1 a %2m.", _unitName, _distance];
		};
	} 
	else 
	{
		_hintMsg = "Nessun medico nelle vicinanze.";
	};
